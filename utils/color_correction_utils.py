...
def apply_otsu_thresholding(values):
    ...
    _, thresholded = cv2.threshold(
        gray_values.astype(np.uint8), 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
    )
...
def plot_images(
    image, image_corrected_exposure, corrected_image, corrected_exposure_color_image
):
...
def plot_gray_scale_histograms(rgb_array, title):
    ...
    grayscale_values = (
        0.2989 * rgb_array[:, 0] + 0.5870 * rgb_array[:, 1] + 0.1140 * rgb_array[:, 2]
    )
