    ...
    for root, dirs, files in os.walk(directory_path):
        for file in files:
            if file.endswith('.txt'):
                ...
                train_loss, train_accuracy, test_loss, test_accuracy = extract_metrics_from_file(file_path)
...
def compare_metrics_across_folders(directories):
    ...
    comparison_df = comparison_df.pivot_table(index='file_name', columns='directory', values=['train_loss', 'train_accuracy', 'test_loss', 'test_accuracy'], aggfunc='first')

    comparison_df.columns = [os.path.basename(col[1]) + f'_{col[0]}' for col in comparison_df.columns]
                        ...
                        with open(file_path, 'r') as f:
                            ...
                            for model, train_accuracy, test_accuracy in matches:
                                ...
                                if model not in model_data[directory][file_name]:
                                    model_data[directory][file_name][model] = {"train": [], "test": []}
                                else:
                                ...
                                model_data[directory][file_name][model]["train"].append(float(train_accuracy))
                                model_data[directory][file_name][model]["test"].append(float(test_accuracy))
    ...
    for directory, file_data in model_data.items():
        ...
        for file_name, models in file_data.items():
            for model, accuracies in models.items():
                ...
                combined_data[train_col][file_index] = accuracies["train"][0]  # Use first value
                combined_data[test_col][file_index] = accuracies["test"][0]  # Use first value
