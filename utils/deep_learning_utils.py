...
def model_training(
    X_train, X_val, X_test, y_train, y_val, y_test, num_classes, experiment, output_file
):
    ...
    feature_names = [
        ...
        "Left Sclera Color G",
        "Left Sclera Color B",
        "Right Sclera Color R",
        "Right Sclera Color G",
        "Right Sclera Color B",
        "FIQA Exposure",
        "OFIQ Illumination Uniformity",
    ]

    # Perform permutation importance with the custom scorer
    result = permutation_importance(
        model, X_test, y_test, n_repeats=10, random_state=42, scoring=keras_accuracy
    )
    ...
    conf_matrix = confusion_matrix(
        np.argmax(y_test, axis=1), y_pred_classes, labels=range(1, 10), normalize="true"
    )
